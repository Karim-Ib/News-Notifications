"""
Send qualifying articles to Gemini 2.0 Flash for structured impact scoring.
...
Output schema per article:
{
  ...
  "magnitude":    1-5,
  "confidence":   0.0-1.0,
  "event_type":   str,          e.g. "sanctions", "tanker_seizure", "supply_cut"
  "narrative_key": str,         short snake_case slug for dedup/thread grouping
  "summary":      str           <= 80-char human-readable headline
}
"""
...
OUTPUT a JSON object with EXACTLY these fields:
  ...
  magnitude     : integer 1-5
                    1 = noise / already priced in
                    2 = minor, <1% price move expected
                    3 = moderate, 1-3% move
                    4 = significant, 3-6% move
                    5 = market-moving, >6% or structural shift
  confidence    : float 0.0-1.0 (your certainty given available information)
...
# Retry settings for Gemini rate-limit responses
_MAX_RETRIES = 3
...
def _parse_response(text: str) -> Optional[dict]:
    ...
    try:
        obj["magnitude"] = max(1, min(5, int(obj["magnitude"])))
...
async def score_article(
    ...
    # same budget as output tokens. 512 was too small -- the model exhausted the
    # budget on reasoning before finishing the JSON. 8192 gives ample headroom.
    # thinking_budget=0 disables internal reasoning for this structured task,
    # making responses faster, cheaper, and deterministic.
    ...
    *,
    ...
    try:
        ...
        for row in rows:
            ...
            # Polite inter-request delay to stay within free-tier RPM (15 req/min)
